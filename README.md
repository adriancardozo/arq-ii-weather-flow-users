# Weather Flow - Wheather (Arquitectura de Software II)
...
```bash
docker build --pull --rm -f 'Dockerfile' -t 'weather-flow-users:latest' '.'
```
...
```bash
docker run -p "3000:3000" --env-file ".env" weather-flow-users
```
